...
{
    public class GetProductsByIdHandler(IRepositoryManager repositoryManager) : IRequestHandler<GetProductsByIdQuery, IEnumerable<ProductDto>>
    {
        ...
        public async Task<IEnumerable<ProductDto>> Handle(GetProductsByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            return products.Select(p => new ProductDto(
                ...
                p.Category,
                p.UserId,
                p.Inventory?.Quantity ?? 0 // <-- return stock
            ));
        }
    }
}
